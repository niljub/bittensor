...
import asyncio
import threading
from bittensor.staging.netman import NetworkManager
...
import requests
from typing import Optional, Union, List, Any, Dict, Callable, Coroutine, Tuple, Type, TypeVar
...
from substrateinterface.exceptions import SubstrateRequestException, ConfigurationError, StorageFunctionNotFound, BlockNotFound, \
...
class SubstrateInterface:
    ...
    async def query_supported_rpc_methods(self):
        await self.async_rpc_request("rpc_methods", [], self.register_supported_rpc_methods)

    async def register_supported_rpc_methods(self, response_data: dict) -> None:
        self.supported_rpc_methods = response_data.get("result", None)
    ...
    async def supports_rpc_method(self, name: str) -> bool:
        ...
        bool
        """
        return name in self.supported_rpc_methods

    async def async_rpc_request(
        self,
        method: str,
        params: List[Any],
        result_handler: Type[Coroutine[Any, Any, None]],
    ) -> None:
        """
        ...
        await self.network.send_data(payload, callback=result_handler)

    def rpc_request(self, *args, **kwargs) -> Dict:
        return self.sync_rpc_request(*args, **kwargs)

    def sync_rpc_request(
            self,
            method: str,
            params: List[Any],
        ...
        asyncio.run_coroutine_threadsafe(
            self.network.send_data(payload, callback=handle_response),
            self.loop
        )
    ...
    def legacy_rpc_request(self, method, params, result_handler=None):
        """
